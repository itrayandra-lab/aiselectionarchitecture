...
- Jika merujuk ke percakapan sebelumnya, lakukan dengan natural`)}`,s=await fetch(r);if(!s.ok)throw new Error(`HTTP ${s.status}`);let i=await s.text();return i=this.cleanPollinationsAds(i),i}cleanPollinationsAds(e){return e.replace(/---Support Pollinations\.AI:---🌸 Ad 🌸Powered by Pollinations\.AI free text APIs\. Support our mission to keep AI accessible for everyone\./gi,"").replace(/---\s*Support\s+Pollinations\.AI\s*:?\s*---\s*🌸\s*Ad\s*🌸.*?Pollinations\.AI.*?everyone\.?/gi,"").replace(/---\s*Support\s+Pollinations\.AI\s*:?\s*---/gi,"").replace(/🌸\s*Ad\s*🌸.*?Pollinations\.AI.*?everyone\.?/gi,"").replace(/🌸\s*Ad\s*🌸/gi,"").replace(/Powered\s+by\s+Pollinations\.AI.*?everyone\.?/gi,"").replace(/Powered\s+by\s+Pollinations\.AI.*?APIs\.?/gi,"").replace(/Support\s+our\s+mission\s+to\s+keep\s+AI\s+accessible\s+for\s+everyone\.?/gi,"").replace(/Pollinations\.AI\s+free\s+text\s+APIs\.?/gi,"").replace(/🌸/g,"").replace(/---+/g,"").replace(/\n{3,}/g,`

`).replace(/\s{3,}/g," ").replace(/^\s+|\s+$/g,"").trim()}addMessage(e,n,r=null){const s=$("#chatMessages"),i=new Date().toLocaleTimeString("id-ID",{hour:"2-digit",minute:"2-digit"});let o="";if(e==="user"){let l="";r&&(l=`
                    <div style="
...
`,document.head.appendChild(t)};document.readyState==="loading"?document.addEventListener("DOMContentLoaded",Ee):Ee();
