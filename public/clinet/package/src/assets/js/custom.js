...
var BeautyZone = function(){
    ...
    if (jQuery("#masonry, .masonry").length) {
		var masonry1 = jQuery("#masonry, .masonry");
	...
	if (jQuery("#masonry2, .masonry2").length) {
		var masonry2 = jQuery("#masonry2, .masonry2");
		if (jQuery(".card-container").length) {
	...
	var handleRemoveBtn = function(){
		document.addEventListener('click', function (e) {
			if (e.target.classList.contains('alert-close')) {
				...
				if (alertBox) {
					alertBox.style.transition = 'opacity 0.3s'; // Fade-out duration
	...
	var handleHeaderMenu = function() {
		...
		var collapseBtns = document.querySelectorAll('.collapse-btn1');
		collapseBtns.forEach((btn) => {
			btn.addEventListener("click", function() {
				...
				if(this.classList.contains('open')){
					...
					collapseBtns.forEach(function(el) {
	...
	var iconstars = function() {
		if(jQuery('.star-rating').length > 0) {
		 stars = new StarRating('.star-rating');
		};
	}
	
	var flatpickrfunction = function() {
