...
export class DraggableAIChat {
    ...
    copyMessageWithFormat(message, buttonElement) {
        // Convert message to Summernote-compatible HTML
        const summernoteHTML = this.formatForSummernote(message);
        
        // Copy both HTML and plain text to clipboard for better compatibility
    ...
    formatForSummernote(text) {
        // Format text to be compatible with Summernote editor
        let summernoteHTML = this.formatResponse(text);
        
        // Clean up and optimize for Summernote
    ...
    async fallbackCopyHTML(htmlContent, plainText) {
        return new Promise((resolve, reject) => {
            // Create a temporary textarea for HTML content
    ...
    async sendMessage() {
        ...
        // Build contextual prompt with history and reply context
        if (this.replyText) {
            fullMessage = this.buildContextualPrompt(message, this.replyText);
            this.closeReply();
        } else {
            // Include chat history even without specific reply
        ...
        try {
            ...
            // Store in history with cleaned text for better context
            const chatEntry = {
    ...
    getRecentHistory() {
        ...
        // Get last 5 chat pairs for context (configurable)
    ...
    buildContextualPrompt(currentMessage, replyContext = null) {
        ...
        // Add reply context if replying to specific message
        if (replyContext) {
    ...
    async callAI(prompt) {
        const systemPrompt = `Anda adalah Ray AI, asisten virtual yang membantu menjawab pertanyaan dalam Bahasa Indonesia. 

ATURAN IDENTITAS:
- Jika ditanya siapa Anda, jawab: "Saya Ray AI, asisten virtual yang siap membantu Anda"
